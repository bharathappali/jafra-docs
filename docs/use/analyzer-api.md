...
`last` accepts seconds, minutes, hours, or days up to seven days. Absolute
values use ISO-8601. Do not combine the `last`, `from`/`to`, `before`, and
`after` families. A valid window with no overlapping recording returns
`404`.

Time-window analysis selects every overlapping rotation and temporarily
concatenates multiple stitched recordings when necessary.
